...
```text
帮我安装 rarevisual-social-card-skill。请把 https://github.com/Jefflaw3333/Rarevisual-social-card-skill 克隆到 ~/.hermes/skills/rarevisual-social-card-skill (或 ~/.claude/skills/rarevisual-social-card-skill),安装完成后检查 SKILL.md、assets/、references/ 是否存在。
```

已经安装过的话,用这段话更新:

```text
帮我更新 rarevisual-social-card-skill。请进入 ~/.hermes/skills/rarevisual-social-card-skill 执行 git pull,然后告诉我当前最新 commit。
```

安装后直接对 Agent 说:

```text
帮我做一套小红书图文 5 张,主题 warm-brass 杂志风,讲一篇关于实心金箔 vs 镀金的科普文。
```

也可以试这些请求(Rare Visual 品牌调性场景):

```text
帮我做一份母亲节礼盒攻略,rv-gold 瑞士风,8 款单品,按送礼对象分组。
把这篇"5 件单品撑起整个工作周"的笔记,做成 ivory 主题 6 页图文,每页一个搭配场景。
我有 3 张项链产品图(都是 gold-filled),帮我做一套 warm-brass 主题的 cover + 工艺幕后 4 页图文。
把这段视频(咖啡馆拍摄 5 秒)做成 Live Photo 动图,主题暖黄铜,文字压在画面左下安静区。
把这次 Q4 限时发售的 4 款新品,做成蜜金色 honey 瑞士风倒计时图文 5 页,含"最后 8 件"紧迫感。
帮我做一份给男友看的礼物选购指南,rv-gold 瑞士风,3 款单品 × 3 种价位,主图用杂志风。
```
...
- 🎨 **10 套主题预设**(Rare Visual 调色板):Editorial 6 套(暖黄铜 · 古铜黄铜 · 瓷青蓝 · 雾橄榄 · 象牙白 · **暗夜黄铜** 暗色)+ Swiss 4 套锚点色(RV 古铜金 · 蜜金色 · 鼠尾草 · 陶土红)
...
| 产品测评 / 工具回顾 | Swiss + RV 古铜金,优先 `S09 KPI Tower` / `S10 H-Bar Chart` |
| 旅行 / 生活方式分享 | Editorial + 暗夜黄铜 或 古铜黄铜,`M16 Image-Led Cover` 满铺主图 |
| 公众号封面对 | 同一份内容渲两块:`.poster.wide` 21:9 + `.poster.square` 1:1,视觉一致 |
| 截图教程 / 工具说明 | `.frame-shot` + `.device-browser` 包壳,优先 Swiss 网格底 |
| 游戏攻略 / 影视回顾 | Editorial + 暗夜黄铜,从 Wallhaven 取游戏原画做满铺 |
| 用户视频 → Live Photo | 先判断 `3s/5s` 信息量,再选单视频 / 拼图 / 三连 / 长视频诊断 |
| 数据回顾 / 年终总结 | Swiss + 蜜金色 或 陶土红,矩阵 + ledger 组合 |
...
| 主题 | 色调 | 适合场景 |
|------|------|---------|
| 🟡 **暖黄铜 Warm Brass** | `#f5efe4` / `#b8924a` | 默认 — 产品摄影、金箔/黄铜特写、礼盒封面 |
| 🟤 **古铜黄铜 Burnished Brass** | `#ece1c8` / `#8f6a2e` | 系列发布、年度精选、陈年厚重调 |
| 🔵 **瓷青蓝 Stone Blue** | `#f2f4f5` / `#315d93` | 材质科普、FAQ、对比类、可信感优先 |
| 🟢 **雾橄榄 Olive Stone** | `#f1ede0` / `#6b7f4a` | 自然 / 半宝石(苔藓玛瑙 / 翡翠 / 拉长石)、慢节奏品牌叙事 |
| 🤍 **象牙白 Ivory** | `#faf6ee` / `#c3a263` | 简洁电商产品 grid、礼盒 line-up、产品图最亮 |
| ⚫ **暗夜黄铜 Midnight Brass** | `#14110d` / `#d4a04a` | 唯一暗调 — 丝绒首饰托盘、烛光礼盒、节日限定 drop |

### Swiss 4 套

| 主题 | 锚点色 | 适合场景 |
|------|--------|---------|
| 🔵 **RV 古铜金 RV Gold** | `#b8924a` | 默认 — 礼盒攻略 / FAQ / 复盘 |
| 🟡 **蜜金色 Honey** | `#e8b54d` | Q4 礼盒季、节日倒计时、温暖能量 |
| 🟢 **鼠尾草 Sage** | `#8aa178` | 环保 / 半宝石 / 春款 / 自然材质 |
| 🟠 **陶土红 Brick** | `#b85542` | 限量发售、紧急提醒、Q4 最后召唤 |
